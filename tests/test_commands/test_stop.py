...
class TestStop:
    # region Capacity
    def test_stop_capacity_success(
        self,
        ...
        # Execute command
        cli_executor.exec_command(f"stop {capacity.full_path} --force")
    ...
    def test_stop_capacity_without_force_success(
        self,
        virtual_workspace_item_factory,
        cli_executor,
        setup_config_values_for_capacity,
        ...
        with patch("questionary.confirm") as mock_confirm:
            ...
            # Execute command
            cli_executor.exec_command(f"stop {capacity.full_path}")

        # Assert
        mock_print_done.assert_called_once()
        assert capacity.display_name in mock_print_done.call_args[0][0]

        mock_questionary_confirm.assert_not_called()  # force is disabled
    ...
    def test_stop_capacity_without_force_cancel_operation_success(
        self,
        virtual_workspace_item_factory,
        cli_executor,
        setup_config_values_for_capacity,
        ...
        get(capacity.full_path, query=".")
        mock_questionary_print.assert_called_once()
        assert '"state": "Paused"' not in mock_questionary_print.call_args[0][0]

    def test_stop_capacity_already_paused_failure(
        self,
        virtual_workspace_item_factory,
        cli_executor,
        ...
        # Assert
        assert_fabric_cli_error(constant.ERROR_NOT_RUNNING)

    # endregion

    # region Mirrored DB
    def test_stop_mirrored_db_success(
        self,
        item_factory,
        cli_executor,
        mock_print_done,
        ...
        # The status is starting, so we need to wait until it is running
        time.sleep(30)

        # Execute command
        cli_executor.exec_command(f"stop {mirrored_db.full_path} --force")
        ...
        mock_questionary_print.assert_called_once()
        # Status can be Stopping or Stopped
        assert (
            '"status": "Stopping"' in mock_questionary_print.call_args[0][0]
            or '"status": "Stopped"' in mock_questionary_print.call_args[0][0]
        )

    def test_stop_mirrored_db_already_paused_failure(
        ...
        # The status is starting, so we need to wait until it is running
        time.sleep(30)
        # Execute command now that the state is Running
        stop(mirrored_db.full_path)
