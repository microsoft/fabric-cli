...
class TestTablesSchemaIntegration:
    ...
    def test_table_schema_dispatches_to_schema_command(self, cli_executor: CLIExecutor):
        with patch(_SCHEMA_COMMAND) as mock_cmd:
            cli_executor.exec_command(
                "table schema /ws.Workspace/lh.Lakehouse/Tables/my_table"
            )
    ...
    def test_table_schema_dispatches_with_schema_namespace(
        self, cli_executor: CLIExecutor
    ):
        with patch(_SCHEMA_COMMAND) as mock_cmd:
            cli_executor.exec_command(
                "table schema /ws.Workspace/lh.Lakehouse/Tables/dbo/my_table"
            )
